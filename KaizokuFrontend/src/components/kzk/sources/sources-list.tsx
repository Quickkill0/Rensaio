...
export function SourcesList({
  ...
  const handleHealthCheck = async (extension: Provider) => {
    try {
      ...
      // Full refresh to get updated health status
      const data = await providerService.getProviders();
      setExtensions(data);
    } catch (error) {
      console.error('Failed to check provider health:', error);
      toast({ title: 'Failed to check source health', variant: 'destructive' });
    } finally {
  ...
  const handleCheckAll = async () => {
    try {
      ...
      toast({ title: 'Failed to check all sources', variant: 'destructive' });
    } finally {
  ...
  return (
    <div className="space-y-8">
      ...
      <SourcesToolbar
        ...
        sort={sort}
        setSort={setSort}
        onCheckAll={() => void handleCheckAll()}
        ...
        >
          {visibleInstalled.map((ext) => (
            <SourceRow
              ...
              onHealthCheck={(extension) => void handleHealthCheck(extension)}
              isLoading={actionLoading === ext.package}
              isHealthChecking={
                healthCheckingPkg === ext.package || isCheckingAll
              }
